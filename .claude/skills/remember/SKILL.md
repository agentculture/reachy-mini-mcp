...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope reachy-mini-mcp
...
scope:** the default personal scope is **private** (`--scope reachy-mini-mcp
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `reachy-mini-mcp`). Private records are served only to a recall in the same
